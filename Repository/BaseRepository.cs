...
using UtilsStandardLib;
using UtilsStandardLib.Json;
using UtilsStandardLib.System;
...
namespace Repository
{
    public class BaseRepository<T> : IBaseRepository<T> where T : class, new()
    {
        ...
        public List<T> QueryableToListOrder(Expression<Func<T, bool>> expression, Expression<Func<T, object>> sort_expression, string orderByType, int num)
        {
            ...
            if (num < 0)
            {
                return _db.Queryable<T>().Where(expression).OrderBy(sort_expression, _orderByType).ToList();
            }
            else
            {
                if (num == 0)
                ...
                return _db.Queryable<T>().Where(expression).OrderBy(sort_expression, _orderByType).Take(num).ToList();
            }
        }
